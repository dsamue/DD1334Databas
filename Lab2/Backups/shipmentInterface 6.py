...
class DBContext:
    ...
    def makeShipments(self):
        ...
        #--Decrease stock but catch exceptions and rollback the transaction if any problem
        try:
            self.cur.execute(query)
        ...
        #Create wuery for new shipment
        query="""INSERT INTO shipments VALUES (%i, %i, '%s','%s');"""%(SID,CID,Sisbn,Sdate)
        print query

        #---Add to shipments but catch exceptions and rollback the transaction in any problem
        try:
            self.cur.execute(query)
